...
#define CPU_TRANSPOSE
    ...
    std::cout << "***Device To Device Copy***" << std::endl;
    {
        ...
        const int BS_X = 32;
        const int BS_Y = 32;
        int gridDimX = (sizeX + BS_X - 1) / BS_X;
        int gridDimY = (sizeY + BS_Y - 1) / BS_Y;

        DIMS dims;
        dims.dimBlock = dim3(BS_X, BS_Y, 1);
        dims.dimGrid  = dim3(gridDimX,
                             gridDimY,
                             1);
    ...
    std::cout << "***Shared Memory Transpose***" << std::endl;
    {
        ...
        {
            // Launch the GPU kernel
            matrixTransposeShared<BS_X, BS_Y><<<dims.dimGrid, dims.dimBlock>>>(d_a, d_b);
        }
        // stop the timer
        cudaEventRecord(stop, 0);
        cudaEventSynchronize(stop);
